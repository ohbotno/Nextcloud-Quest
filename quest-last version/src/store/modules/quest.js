...
const state = {
    ...
    stats: {
        level: {
            ...
            // Align keys with UI components' expectations
            current_xp: 0,
            lifetime_xp: 0,
...
const mutations = {
    ...
    setStats(state, stats) {
        // Deep-merge known nested objects to avoid wiping fields on partial updates
        const incomingLevel = stats.level || {}
        const incomingStreak = stats.streak || {}
        const incomingAchievements = stats.achievements || {}

        state.stats = {
            ...state.stats,
            ...stats,
            level: { ...state.stats.level, ...incomingLevel },
            streak: { ...state.stats.streak, ...incomingStreak },
            achievements: { ...state.stats.achievements, ...incomingAchievements }
        }
    },
...
const actions = {
    ...
    async completeTask({ commit }, { taskId, taskTitle, priority = 'medium' }) {
        ...
        try {
            ...
            if (response.status === 'success') {
                ...
                // Let QuestApp/StatsService be the single source of truth
                if (typeof window !== 'undefined' && window.QuestApp) {
                    try { await window.QuestApp.refreshStats() } catch (e) { /* ignore */ }
                }
                ...
                // Stats refresh handled by QuestApp; no duplicate refresh here
