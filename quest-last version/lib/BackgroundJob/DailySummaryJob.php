...
class DailySummaryJob extends TimedJob {
    ...
    protected function run($argument): void {
        try {
            $this->logger->info('Starting daily summary and reset job');
            
            // First, perform daily resets for all users
            $resetResults = $this->performDailyResets();
            ...
            $this->logger->info('Daily summary and reset job completed', [
                'summaries_sent' => $summariesSent,
                'total_users' => count($users),
                'users_reset' => $resetResults['users_reset']
            ]);
    ...
    /**
     * Perform daily resets for all users
     * Reset daily counters and update timestamps
     */
    private function performDailyResets(): array {
        $results = ['users_reset' => 0, 'errors' => 0];
        $today = date('Y-m-d');
        
        try {
            // Get all users in ncquest_users table
            $allQuests = $this->questMapper->findAll();
            ...
            foreach ($allQuests as $quest) {
                try {
                    ...
                    // Check if reset needed (last_daily_reset is not today)
                    $lastReset = $quest->getLastDailyReset();
                    if ($lastReset && date('Y-m-d', strtotime($lastReset)) === $today) {
                        continue; // Already reset today
                    }
                    
                    // Perform the reset
                    $quest->setTasksCompletedToday(0);
                    $quest->setXpGainedToday(0);
                    $quest->setLastDailyReset(date('Y-m-d H:i:s'));
                    
                    $this->questMapper->update($quest);
                    $results['users_reset']++;
                    
                    $this->logger->debug('Daily reset performed for user', [
                        'user_id' => $userId,
                        'reset_date' => $today
                    ]);
                    ...
                    $results['errors']++;
                    $this->logger->error('Failed to reset daily counters for user', [
                        'user_id' => $quest->getUserId() ?? 'unknown',
                        'error' => $e->getMessage()
                    ]);
                }
            }
            
            $this->logger->info('Daily reset completed', [
                'users_reset' => $results['users_reset'],
                'errors' => $results['errors']
            ]);
            
        } catch (\Exception $e) {
            $this->logger->error('Daily reset failed', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
        }
