...
abstract class BasePageController extends Controller {
    ...
    protected function renderPage(string $pageName, string $templateName, array $additionalScripts = [], array $additionalStyles = []): TemplateResponse {
        ...
        // Add core architecture scripts (always needed) - ORDER MATTERS!
        // Cache-busting to force reload of debug-enabled files
        $cacheVersion = '?v=' . time();
        Util::addScript('quest', 'core/stats-service' . $cacheVersion);
        Util::addScript('quest', 'core/dom-updater' . $cacheVersion);
        Util::addScript('quest', 'core/quest-app' . $cacheVersion);
